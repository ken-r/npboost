...
for model in "npboost" "anpboost" "np" "anp"; do
    for effect in "steps_1D" "steps_2D"; do
        ...
        screen -dmS "brownian_lognormal_${model}_${effect}_rmse_012" bash -c '
...
for model in "npboost" "anpboost" "np" "anp"; do
    for effect in "steps_1D" "steps_2D"; do
        export SCREEN_MODEL=$model
        export SCREEN_EFFECT=$effect
        screen -dmS "brownian_lognormal_${model}_${effect}_rmse_34" bash -c '
            for seed in 3 4; do
                for task in "in_context" "few_shot"; do
                    echo "Running experiment for model ${SCREEN_MODEL}, effect ${SCREEN_EFFECT}, seed 0, task $task..."
                    CUDA_VISIBLE_DEVICES=1 nice -7 python -m scripts.experiments.run_experiment \
...
for model in "lme" "gplinear"; do
    export SCREEN_MODEL=$model
    screen -dmS "brownian_lognormal_${model}_rmse" bash -c '
        for effect in "steps_1D" "steps_2D"; do
...
for model in "gbm_no_group" "gbm_group_cat"; do
    for effect in "steps_1D" "steps_2D"; do
        ...
        screen -dmS "brownian_lognormal_${model}_${effect}_rmse" bash -c '
            for seed in 0 1 2 3 4; do
                for task in "in_context" "few_shot"; do
